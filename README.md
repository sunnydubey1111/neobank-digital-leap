...
bank** — carrying a COBOL/z-OS mainframe core into a hybrid on-premises and cloud platform that
serves a million customers in real time, without ever putting the ledger at risk.
...
| **Context** | A publicly traded retail bank — 1M customers, 200 branches, founded 1905 — must go digital or lose to fintechs and open-banking rivals |
...
| **Outcome** | A read/write-split hybrid architecture that removes 93% of the mainframe operations the digital channel would otherwise consume — roughly $3.2M per year at year-three volume |

## The problem

NeoBank's Core Banking System is COBOL on z/OS, with DB2 holding transactions and ADABAS
holding customers. It delivers exactly what a bank is legally obliged to deliver — strong
consistency, an accurate ledger, full audit trails, 99.999% reliability — and it charges for
every operation, running to millions of dollars a year.
...
channels shipped in weeks — while every money movement remains a Core Banking System
transaction.
...
derived read model, continuously fed by change data capture from DB2 and ADABAS over a
replayable event log. Reads never touch the mainframe.
...
| | |
|---|---|
| **Cost** | 290M → 20M mainframe operations per month at year three — a 93% reduction, ~$3.24M/year, and the dominant cost lever across the entire plausible range of mainframe unit pricing |
| **Latency** | p95 of 55 ms on a balance read, against a 100 ms budget, because no request waits on a mainframe |
| **Availability** | The read path has no dependency on the core, so it can genuinely be designed for five nines even though the core-dependent write path cannot |
...
| **[Decision Log](docs/solution/decisions.md)** | Fourteen architecture decisions, each with the options genuinely considered and the trade-off accepted |
| **[Diagrams](docs/solution/diagrams.md)** | C4 context and container, data architecture, network topology, six flow sequences, the transfer state machine, and the scaling view — all as Mermaid |
| **[Delivery Roadmap](ROADMAP.md)** | How 11,450 developer-days across fourteen teams fit into twelve months |
...
8. **Availability is tiered by path, not averaged** — a blended five-nines figure would be a fiction.
...
<sub>Produced for the Global Dev Experts *Software Architect* programme. This repository
contains only original work; the copyrighted course slides, recordings and summaries are
deliberately excluded.</sub>
